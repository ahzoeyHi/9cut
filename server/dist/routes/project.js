...
router.get('/:id', (req, res) => {
    try {
        ...
        const storyboards = storyboard_1.storyboardModel.findByProjectId(req.params.id);
        res.json({
...
router.put('/:id', (req, res) => {
    try {
        ...
        const project = project_1.projectModel.update(req.params.id, {
            name,
            description,
            script,
            status
...
router.delete('/:id', (req, res) => {
    try {
        ...
        resource_1.resourceModel.deleteByProjectId(projectId);
        // 删除分镜
        storyboard_1.storyboardModel.deleteByProjectId(projectId);
        // 删除项目
        const success = project_1.projectModel.delete(projectId);
        res.json({ success });
    }
    catch (error) {
