...
router.get('/tasks/:taskId', (req, res) => {
    try {
        const task = task_1.taskModel.findById(req.params.taskId);
        if (!task) {
...
router.get('/projects/:projectId/tasks', (req, res) => {
    try {
        const tasks = task_1.taskModel.findByProjectId(req.params.projectId);
        res.json({
...
// 生成分镜图片
router.post('/storyboards/:id/generate/image', (req, res) => {
    try {
        const storyboardId = req.params.id;
        const task = task_1.taskModel.create({
            type: 'image',
...
// 生成分镜视频
router.post('/storyboards/:id/generate/video', (req, res) => {
    try {
        const storyboardId = req.params.id;
        const task = task_1.taskModel.create({
            type: 'video',
...
// 生成口播文案
router.post('/projects/:projectId/generate/narration', (req, res) => {
    try {
...
// 拆分口播文案
router.post('/projects/:projectId/narration/split', (req, res) => {
    try {
        // TODO: 实际的拆分逻辑
...
// 生成语音
router.post('/storyboards/:id/generate/speech', (req, res) => {
    try {
        const storyboardId = req.params.id;
        const task = task_1.taskModel.create({
            type: 'speech',
...
// 合并视频
router.post('/projects/:projectId/merge-video', (req, res) => {
    try {
        const projectId = req.params.projectId;
        const task = task_1.taskModel.create({
            type: 'merge',
...
// 批量生成
router.post('/projects/:projectId/generate/batch', (req, res) => {
    try {
        const projectId = req.params.projectId;
