...
async function fetchOpenAIModels(apiKey, endpoint) {
    ...
    const configuredModels = (0, modelConfig_1.getConfiguredModels)('openai');
    try {
        ...
        const models = data.data.map(model => {
            ...
            return {
                id: model.id,
                name: model.id,
                capabilities: ['text', 'image', 'speech', 'video']
            };
        });
 ...
 */
async function fetchClaudeModels(_apiKey, _endpoint) {
    return (0, modelConfig_1.getConfiguredModels)('claude');
}
/**
 * 获取Gemini模型列表
 */
async function fetchGeminiModels(apiKey, endpoint) {
    const baseUrl = endpoint || 'https://generativelanguage.googleapis.com/v1beta';
    const configuredModels = (0, modelConfig_1.getConfiguredModels)('gemini');
    try {
 ...
 * 获取通义千问模型列表
 */
async function fetchQwenModels(apiKey, endpoint) {
    const baseUrl = endpoint || 'https://dashscope.aliyuncs.com/api/v1';
    const configuredModels = (0, modelConfig_1.getConfiguredModels)('qwen');
    try {
        const response = await fetch(`${baseUrl}/models`, {
            headers: {
                'Authorization': `Bearer ${apiKey}`
 ...
 * 获取火山引擎模型列表
 */
async function fetchVolcengineModels(apiKey, endpoint) {
    const baseUrl = endpoint || 'https://ark.cn-beijing.volces.com/api/v3';
    const configuredModels = (0, modelConfig_1.getConfiguredModels)('volcengine');
    try {
        const response = await fetch(`${baseUrl}/models`, {
            headers: {
                'Authorization': `Bearer ${apiKey}`
 ...
 */
async function fetchKimiModels(apiKey, endpoint) {
    // 默认Kimi API地址
 ...
 */
async function fetchGLMModels(apiKey, endpoint) {
    // 默认智谱API地址
 ...
 */
async function fetchDeepSeekModels(apiKey, endpoint) {
    // 默认DeepSeek API地址
...
async function fetchOpenAICompatibleModels(apiKey, baseUrl, serviceType) {
    const configuredModels = (0, modelConfig_1.getConfiguredModels)(serviceType);
    try {
        const response = await fetch(`${baseUrl}/models`, {
            headers: {
                'Authorization': `Bearer ${apiKey}`
 ...
 * 获取指定服务的模型列表
 */
async function fetchModels(serviceType, apiKey, endpoint) {
    switch (serviceType) {
        case 'openai':
 ...
 * 根据功能类型过滤模型列表
 */
function filterModelsByFunction(models, functionType) {
 ...
 */
async function getAvailableModels(serviceType, _functionType, apiKey, endpoint) {
    return fetchModels(serviceType, apiKey, endpoint);
}
/**
 * 获取配置的模型列表（不调用API，直接返回配置）
 */
function getStaticModels(serviceType) {
    return (0, modelConfig_1.getConfiguredModels)(serviceType);
}
